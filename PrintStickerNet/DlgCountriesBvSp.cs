...
namespace PrintSticker {
    public partial class DlgCountriesBvSp : Form {
        ...
        public DlgCountriesBvSp(bool bBV) {
            _bBV = bBV;
            if (bBV)
                _strFileBdName = "DbCountriesBV.dbf";
            else
                _strFileBdName = "DbCountriesSP.dbf";
        ...
        private void _GetUseColumn(out Dictionary<string, string> hUseColumns) {
            ...
            hUseColumns.Add("NAME", "Название страны");//2
        ...
        private void DlgCityBV_Load(object sender, EventArgs e) {
            if (!_bBV)
                Text = "Справочник стран для Сопутки";
            _ShowView();
        }
        ...
        private void DlgCityBV_FormClosing(object sender, FormClosingEventArgs e) {
            _FormClose();
        }
        ...
        private void menuAdd_Click(object sender, EventArgs e) {
            DlgAddСountryProduct dlg = new("Добавить название страны");
            ...
            foreach (DataGridViewRow row in dataGridView.Rows) {
                if (name.Trim().ToLower() == row.Cells[NAME].Value.ToString().Trim().ToLower() ||
                    fullName.Trim().ToLower() == row.Cells[NAMEFULL].Value.ToString().Trim().ToLower()) {
        ...
        private void contextMenu_Opening(object sender, CancelEventArgs e) {
            if (0 == dataGridView.SelectedRows.Count) {
                ...
                //List<string> listRows = [];
                foreach (DataGridViewRow row in dataGridView.SelectedRows) {
                    if (_strNOTEXIST.ToUpper() == row.Cells[NAME].Value.ToString().ToUpper())
                        return;
                }               
